import { getAllUser, IAdminUser } from "@/app/admin/service/user.service";
...
interface QueryParams {
  ...
  sorters: SorterResult<IAdminUser> | SorterResult<IAdminUser>[];
}
...
const fetchData = async (
  ...
  return await getAllUser(RECORD_STATUS.ACTIVE, page, size);
};
...
const TableView = () => {
  ...
  const { data, isLoading } = useQuery<ApiResponse<IAdminUser>, Error>({
    queryKey: [
      "IAdminUser",
  ...
  const handleTableChange: TableProps<IAdminUser>["onChange"] = (
    pagination,
    filters,
    sorters
  ) => {
    setQueryParams({
      ...
      filters,
      sorters: sorters as SorterResult<IAdminUser> | SorterResult<IAdminUser>[],
    });
  };
